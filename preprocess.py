...
base_path = 'C:/Users/eleju/Desktop/TFM/dataset-fitness/'
# New folder
file_hr_1 = base_path + 'FitabaseData3.12.16-4.11.16-new/heartrate_seconds_merged.csv'
file_min_int_1 = base_path + 'FitabaseData3.12.16-4.11.16-new/minuteIntensitiesNarrow_merged.csv'
file_min_steps_1 = base_path + 'FitabaseData3.12.16-4.11.16-new/minuteStepsNarrow_merged.csv'
# Original folder
file_hr_2 = base_path + 'FitabaseData4.12.16-5.12.16/heartrate_seconds_merged.csv'
file_min_int_2 = base_path + 'FitabaseData4.12.16-5.12.16/minuteIntensitiesNarrow_merged.csv'
file_min_steps_2 = base_path + 'FitabaseData4.12.16-5.12.16/minuteStepsNarrow_merged.csv'


# PREPROCESS FUNCTION

def preprocess(user, file_hr_1, file_hr_2, file_min_int_1, file_min_int_2, file_min_steps_1, file_min_steps_2):
    ...
    # 2022484408: 42 days (33), 2026352035: 5 days (0), 2347167796: 32 days (28), 4020332650: 27 days (10), 
    # 4558609924: 42 days (10), 5553957443: 42 days (35), 5577150313: 39 days (28-error), 6117666160: 31 days (24), 
    # 6775888955: 26 days (3), 6962181067: 44 days (25), 7007744171: 35 days (28), 8792009665: 29 days (22), 
    ...
    # Store the new days used
    dates = [day[0].date() for day in day_time_new]


    # SMOOTHING ---------------------------------------------------------------------------------
    '''
    # Function to perform smoothing with moving average technique depending on z value
    def smoothing_average(z):
        # Equation type x(t) = 1/(2z+1) * sum(x(t+k) for k in range(-z, z))
        # Example for 5 values: x(t) = 1/5 * [x(t-2) + x(t-1) + x(t) + x(t+1) + x(t+2)]
        def equation_average(x_values, z):
            n = len(x_values)
            # Create an extended array to handle the edges using reflection
            day_array_ext = np.pad(x_values, (z//2, z//2), 'reflect')
            x_t = np.zeros(n)
            for t in range(n):
                x_t[t] = np.mean(day_array_ext[t:t+z])
            return x_t

        # New variable for days with the smoothed values of heart rate
        day_hr_smooth = []
        for day in day_hr_new:
            day_array = np.array(day, dtype=float)
            # Apply the formula of average smoothing for each day
            day_smooth = equation_average(day_array, z)
            # Convert the heart rate values to integers
            day_smooth = np.round(day_smooth).astype(int)
            # Add day with smoothed values
            day_hr_smooth.append(day_smooth.tolist())
        return day_hr_smooth

    # Perform smoothing with z surrounding points
    z = 35
    day_hr_smooth = smoothing_average(z)

    # Plot HeartRate before and after Smoothing
    def plot_hr_smoothing(z):
        plt.figure(figsize=(20, 10))
        # Make a plot for each day
        for id,day in enumerate(dates):
            # HeartRate BEFORE Smoothing
            plt.subplot(2, 1, 1)
            x = day_time_new[id]
            y = day_hr_new[id]
            plt.plot(x, y, label='HeartRate BEFORE Smoothing', alpha=0.5)
            plt.xlabel('\nTime')
            plt.ylabel('HeartRate (bpm)\n')
            plt.title(f'HR Before and After Smoothing of user {user} on {day}')
            plt.ylim(30, 220)
            plt.grid(True)
            plt.legend()
            # HeartRate AFTER Smoothing
            plt.subplot(2, 1, 2)
            x = day_time_new[id]
            y = day_hr_smooth[id]
            # Check if the lengths match
            if len(x) != len(y): 
                print(f'*Lengths of heart rates after smoothing do not match - graphic not created')
                return
            plt.plot(x, y, label='HeartRate AFTER Smoothing', color='orange', alpha=0.5)
            plt.xlabel('\nTime')
            plt.ylabel('HeartRate (bpm)\n')
            plt.ylim(30, 220)
            plt.grid(True)
            ...
            if not os.path.exists(f'figures/user{user}'):
                os.makedirs(f'figures/user{user}')
            # Save the figure
            plt.savefig(f'figures/user{user}/hr__smooth_{z}_{day}.png', bbox_inches='tight')
            #plt.show()
            plt.clf()
    ...
    maxx, minn, mean, median, std_deviation, rmssd, pnn50 = calculate_statistics(day_hr_statistics)
    ...
    def relation_hr_int_time():
        ...
        for id,day in enumerate(dates):
            ...
            if len(x) != len(y): 
                print(f'*Lengths of heart rates and intensities do not match - graphic not created')
                return
            plt.scatter(x, y, color='purple', label='Intensity', alpha=0.5)
            plt.xlabel('\nTime')
            plt.ylabel('Intensity (0: sedentary - 3: very active)\n')
            plt.ylim(-0.5, 3.5)
            plt.grid(True)
            ...
            plt.xlabel('\nTime')
            plt.ylabel('HeartRate (bpm)\n')
            plt.title(f'HR and Intensity of user {user} on {day}')
            plt.ylim(30, 220)
            plt.grid(True)
            plt.legend()
            # Adjust space between subplots
    ...
    def relation_hr_steps_time():
        ...
        for id,day in enumerate(dates):
            ...
            if len(x) != len(y): 
                print(f'*Lengths of heart rates and steps do not match - graphic not created')
                return
            plt.plot(x, y, label='Steps', color='purple', alpha=0.5)
            plt.xlabel('\nTime')
            plt.ylabel('Steps\n')
            plt.ylim(-5, 155)
            plt.grid(True)
            ...
            plt.xlabel('\nTime')
            plt.ylabel('HeartRate (bpm)\n')
            plt.title(f'HR and Steps of user {user} on {day}')
            plt.ylim(30, 220)
            plt.grid(True)
            plt.legend()
            # Adjust space between subplots
    ...
    with open(f'datasets/data_user{user}.pkl', 'wb') as f:
        pickle.dump(data, f)

    # Return dataset in JSON format - adapt the time values
    data['Date'] = pd.to_datetime(data['Date']).dt.strftime('%Y-%m-%d')
    data['Time'] = data['Time'].apply(lambda times: [pd.to_datetime(t).strftime('%Y-%m-%d %H:%M:%S') for t in times])
...
def main(user, show_output):
    ...
    result_json = preprocess(user, file_hr_1, file_hr_2, file_min_int_1, file_min_int_2, file_min_steps_1, file_min_steps_2)
